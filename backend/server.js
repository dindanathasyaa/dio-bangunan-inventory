...
app.delete('/api/categories/:id', async (req, res) => {
    ...
    try {
        ...
        if (error.code === 'ER_ROW_IS_REFERENCED_2' || error.errno === 1451 || error.code === 'ER_ROW_IS_REFERENCED' || error.errno === 1217) {
        ...
        for (let item of inventoryRows) {
            ...
            if (item.stock >= item.max_stock) {
                transferSuggestions.push({
                    ...
                    message: `Gudang kepenuhan! Terdapat ${Math.floor(Number(item.stock))} stok, melampaui batas maksimal (${Math.floor(item.max_stock)}). Kurangi order atau adakan promo.`
                });
            }
        }
